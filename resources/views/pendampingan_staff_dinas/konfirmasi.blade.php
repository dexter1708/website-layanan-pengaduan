    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow-md rounded-lg border-2 border-blue-400 overflow-hidden">
            <div class="px-6 py-4 bg-gray-50 border-b">
                <h2 class="text-xl font-bold text-gray-800">Layanan Pendampingan Korban</h2>
            </div>
            <div class="px-6 py-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 text-base">
                    ...
                    <div class="space-y-5">
                        <div>
                            <p class="text-sm text-gray-500 font-semibold">Jenis Pendampingan</p>
                            <p class="text-gray-900 font-semibold">{{ $pendampingan->jenis_layanan }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 font-semibold">Tanggal Pendampingan</p>
                            <p class="text-gray-900 font-semibold">{{ \Carbon\Carbon::parse($pendampingan->tanggal_pendampingan)->isoFormat('dddd, D MMMM Y') }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 font-semibold">Nama Pendamping</p>
                            <p class="text-gray-900 font-semibold">{{ $pendampingan->nama_pendamping }}</p>
                        </div>
                    </div>
                    <!-- Kolom Kanan -->
                    <div class="space-y-5">
                        <div>
                            <p class="text-sm text-gray-500 font-semibold">Waktu Pendampingan</p>
                            <p class="text-gray-900 font-semibold">{{ \Carbon\Carbon::parse($pendampingan->tanggal_pendampingan)->format('H:i') }} WIB</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 font-semibold">Tempat</p>
                            <p class="text-gray-900 font-semibold">{{ $pendampingan->tempat_pendampingan }}</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="bg-gray-50 px-6 py-4">
                <form method="POST" action="{{ route('pendampingan.konfirmasi.update', $pendampingan->id) }}" class="flex items-center justify-center gap-4">
                    @csrf
                    @method('PUT')
