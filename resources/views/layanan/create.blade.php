...
<section class="bg-white py-6 px-4 sm:px-6 lg:px-8">
    ...
    <nav class="text-sm text-gray-600 font-semibold mb-6" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2">
            ...
            <li class="text-gray-600">/</li>
            <li class="text-gray-500">Tambah Layanan</li>
        </ol>
    </nav>

    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Tambah Layanan Baru</h1>
        <p class="text-gray-600">Tambahkan jenis layanan baru untuk konseling dan pendampingan.</p>
    </div>
    ...
    <div class="bg-white shadow-md rounded-lg p-6 max-w-2xl">
        <form method="POST" action="{{ route('staff.layanan.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select id="jenis_layanan" 
                    ...
                    <option value="pendampingan" {{ old('jenis_layanan') == 'pendampingan' ? 'selected' : '' }}>
                        Pendampingan
                    </option>
                    <option value="konseling" {{ old('jenis_layanan') == 'konseling' ? 'selected' : '' }}>
                        Konseling
                    </option>
                </select>
                @error('jenis_layanan')
