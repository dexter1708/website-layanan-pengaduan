            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('staff.pendampingan.update', $pendampingan->id) }}" class="space-y-6">
                                ...
                                @foreach($pengaduans as $pengaduan)
                                    @if($pengaduan->korban->isNotEmpty())
                                        <option value="{{ $pengaduan->id }}" 
                                            ...
                                            {{ $pengaduan->id == $pendampingan->pengaduan_id ? 'selected' : '' }}>
                        ...
                        <!-- Hidden Korban ID -->
                        <input type="hidden" name="korban_id" id="korban_id" value="{{ $pendampingan->korban_id }}">

                        <!-- Nama Pendamping -->
                        <div>
                            <x-input-label for="nama_pendamping" :value="__('Nama Pendamping')" />
                            <x-text-input id="nama_pendamping" class="block mt-1 w-full" type="text" name="nama_pendamping" :value="old('nama_pendamping', $pendampingan->nama_pendamping)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="tanggal_pendampingan" class="block mt-1 w-full" type="datetime-local" name="tanggal_pendampingan" :value="old('tanggal_pendampingan', $pendampingan->tanggal_pendampingan->format('Y-m-d\TH:i'))" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="tempat_pendampingan" class="block mt-1 w-full" type="text" name="tempat_pendampingan" :value="old('tempat_pendampingan', $pendampingan->tempat_pendampingan)" required />
