                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($pendampingans as $pendampingan)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $pendampingan->pengaduan->id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $pendampingan->nama_korban }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $pendampingan->nama_pendamping }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $pendampingan->tanggal_pendampingan->format('d/m/Y H:i') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $pendampingan->tempat_pendampingan }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('pendampingan.show', $pendampingan->id) }}" class="text-blue-600 hover:text-blue-900">Detail</a>
                                                @if(auth()->user()->role === 'staff')
                                                    <a href="{{ route('staff.pendampingan.edit', $pendampingan->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                                ...
                                                @elseif($pendampingan->konfirmasi === 'menunggu')
                                                    <form action="{{ route('pendampingan.update-konfirmasi', $pendampingan->id) }}" method="POST" class="inline">
                                                        ...
                                                        <button type="submit" class="text-green-600 hover:text-green-900 bg-green-100 hover:bg-green-200 px-2 py-1 rounded">Setuju</button>
                                                    </form>
                                                    <form action="{{ route('pendampingan.update-konfirmasi', $pendampingan->id) }}" method="POST" class="inline">
                                                        @csrf
                                                        @method('PUT')
